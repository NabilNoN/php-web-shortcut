...
/**
 * @version 1.5 2020
 * @license nabil.jaran@gmail.com
 * @copyright 2020-07-13
...
class Mysql
{
    ...
    private function getTable($table)
    {
        ...
        $tbl = self::table_prefix();
        if (substr($table, 0, strlen($tbl)) === $tbl) return $table;
    ...
    public function where_is_not_null($column, $type = "AND")
    {
        $where = $this->where;
        if (empty($where)) {
            $where = sprintf("WHERE `%s` %s", $column, "IS NOT NULL");
        } else {
            $where .= sprintf(" %s `%s` %s", $type, $column, "IS NOT NULL");
        }
        $this->where = $where;
        return $this;
    }

    public function or_where_is_not_null($column)
    {
        return self::where_is_not_null($column, $type = "OR");
    }

    public function and_where_is_not_null($column)
    {
        return self::where_is_not_null($column, $type = "AND");
    }

    public function where_is_null($column, $type = "AND")
    {
        $where = $this->where;
        if (empty($where)) {
            $where = sprintf("WHERE `%s` %s", $column, "IS NULL");
        } else {
            $where .= sprintf(" %s `%s` %s", $type, $column, "IS NULL");
        }
        $this->where = $where;
        return $this;
    }
    ...
    {
        return self::where_is_null($column, $type = "OR");
    }

    public function and_where_is_null($column)
    {
        return self::where_is_null($column, $type = "AND");
    }
